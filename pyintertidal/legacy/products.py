...
This is what produced the index comparison: NDWI -0.624, SCL -0.586, AWEI
-0.584, MNDWI -0.559, by Spearman correlation between water frequency and
elevation.

**Treat that ranking as provisional.** The elevation it was scored against is
the IGN 5 m LiDAR, which on a tidal flat records the water surface at flight
time rather than the bed — across our intertidal mask it takes seven distinct
int16 values and 83 % of it sits at exactly +2.00 m. The A/B design here is
sound (one job, byte-identical dates and cloud masks), so re-running it
against a float-precision reference would settle the order; until then the
independent evidence for NDWI is the Bay of Santander comparison, where NDWI
and SCL agree on 91.7 % of pixels and MNDWI claims five times the intertidal
area.
