...
``optimized``  bracketing + energy optimisation + diffusion
``pixels``     pure bracketing (midpoint of the tide bracket)
``isolines``   waterline interpolation
``step``       DEA-style dry→wet crossing (also available as a local,
               cloud-free implementation in
               :func:`pyintertidal.elevation.fit_step`, which is what new
               work should use)
``siq``        Soft Inundation Quantile, an experiment that inverts the tide
               CDF from a soft inundation frequency

.. warning::

   This docstring used to rank these methods by RMSE against the IGN 5 m
   LiDAR at Villaviciosa — 1.14 m, 3.35 m, 3.39 m, 0.85 m, against 0.76 m for
   HSR. **Those numbers are withdrawn.** On a tidal flat that LiDAR records
   the water surface at the moment of the flight, not the bed: across our
   intertidal mask it takes seven distinct int16 values and 83 % of it sits
   at exactly +2.00 m. A ranking scored on it is uninformative about the
   methods. The bracketing estimators here are still expected to be far
   coarser than a full-curve fit — that is a property of throwing away every
   observation except two — but the package should not quote a figure it
   cannot stand behind.
