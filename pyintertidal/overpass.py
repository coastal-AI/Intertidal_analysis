...
def _session_with_retries():
    ...
    Certificate validation is routed through the operating system's trust
    store when ``truststore`` is available — on managed Windows machines the
    bundled CA list does not include the corporate root, and the request
    would otherwise fail with a certificate error even though the network is
    fine. :func:`pyintertidal.scenes.connect` does the same for openEO.
    """
    try:
        import truststore
