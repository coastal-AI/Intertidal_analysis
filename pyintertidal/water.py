...
``ndwi``  (default) — NDWI = (B03 − B08)/(B03 + B08). Both bands are native
    10 m, so this is the only index with TRUE 10 m resolution: nothing is
    resampled across the waterline, which is exactly where the signal lives.
    Threshold ≈ 0.0 for turbid estuaries (Cantabrian rías), ≈ 0.1 for clear
    water.
