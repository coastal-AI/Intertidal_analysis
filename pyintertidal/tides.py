...
2. **How high was the tide?** A global tide model evaluated with pyTMD.
   ``model`` is selectable; the default is ``"EOT20"`` (1/8°), which is also
   what DEA Intertidal uses. ``"GOT4.10"`` remains available and needs no
   manual install, but its 0.5° grid is coarse enough to matter in an
   estuary — at the Ría de Villaviciosa its nearest ocean cell is **34 km
   away** against EOT20's 13 km, so every elevation fitted on it used a tide
   imported from open water. Measure it for your own site with
   :func:`pyintertidal.tidecheck.nearest_ocean_km`.
   FES2014 / TPXO9 work when their files are present in ``directory``.
   The prediction point is user-given or the AOI centroid
   (``location="auto"``).
...
class TideService:
    ...
    ----------
    model : str
        Tide model name understood by pyTMD (default ``"EOT20"``).
    directory : str
    ...
    def __init__(self, model="EOT20", directory="./tide_models",
    ...
    def _tide_model(self, verbose=True):
        ...
        if self._model is None:
            if self.backend == "cmems":
                ...
                # A 2° box is wide enough to find water almost anywhere — and
                # that is the danger: it silently succeeded at Villaviciosa by
                # reaching an ocean cell 32 km away, and nothing said so. The
                # search stays, but `nearest_ocean_km` makes the distance
                # visible; see pyintertidal.tidecheck.
