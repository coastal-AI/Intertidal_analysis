...
:class:`PyTMDTideModel`
    Harmonic global tide models via pyTMD. ``GOT4.10`` (NASA GSFC) needs no
    credentials and downloads itself on first use — this is the default and
    what our published results use. ``FES2014``/``FES2022`` (AVISO/CNES) are
...
class PyTMDTideModel:
    ...
    _MODEL_DIRS = {"GOT4.10": "GOT4.10c", "FES2022": "fes2022b",
                   "FES2014": "fes2014"}
    ...
    def _elevations(self, lons, lats, times):
        ...
        import pyTMD
        return pyTMD.compute.tide_elevations(
            lons, lats, times, model=self._pytmd_name,
            directory=self.directory, crs="4326", standard="datetime",
        )
    ...
    def get_tide_height(self, lat, lon, dt):
        ...
        heights = self._elevations(flat_lons, flat_lats, times).values
    ...
    def get_tide_heights_batch(self, lat, lon, datetimes):
        ...
        for probe in (0.2, 0.5, 1.0, self.box_size):
            ...
            t0 = np.array([np.datetime64(datetimes[0])] * flat_lats.size,
                          dtype="datetime64[ns]")
            heights = self._elevations(flat_lons, flat_lats, t0).values
            valid = ~np.isnan(heights)
            if valid.any():
                dx = flat_lons[valid] - lon
                dy = flat_lats[valid] - lat
