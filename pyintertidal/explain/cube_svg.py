...
def draw_cube(bands, n_dates, shape, thumbnails=None, dates=None,
    ...
    # Column headers: the temporal dimension.
    c.text(left, top - 16, "t  (time)", size=9, fill=MUTED)
    for j in range(n_cols):
        x = left + j * (cell + gap)
        label = ""
        if dates is not None and j < len(dates):
            label = str(dates[j])[5:]              # MM-DD is enough
        elif n_dates:
            label = f"t{j}"
        c.text(x + cell / 2, top - 5, label, size=7.5, fill=MUTED,
