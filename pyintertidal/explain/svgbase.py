...
#: Panel colour painted behind every diagram. Notebooks are read in light
#: AND dark themes, and an SVG inherits neither: without an explicit
#: background, dark text lands on a dark page and becomes unreadable.
#: Painting the panel makes a diagram look identical in both themes.
BACKGROUND = "#ffffff"
...
class Canvas:
    ...
    The canvas paints an opaque background by default so the diagram reads
    the same in a light or a dark notebook theme; pass ``background=None``
    to make it transparent and inherit the page.
    """
    ...
    def __init__(self, width, height, font="system-ui, sans-serif",
        ...
        self.parts = []
        if background:
            self.rect(0, 0, width, height, fill=background, rx=6)
