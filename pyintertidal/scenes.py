...
def connect(backend="openeo.dataspace.copernicus.eu", fix_ssl=True):
    ...
    system's trust store (``truststore``), which is what Windows corporate
    machines need — without it the connection fails with a certificate
    error. The first call opens a browser for OIDC login; the token is then
    cached by the openeo client.
    """
    if fix_ssl:
        try:
            import truststore
